...
if __name__ == "__main__":
    ...
    dataDir = 'C:/Users/xxxxx/OneDrive/Desktop/Thesis Repo/PhyCNN/'
    #################################################################################
    # Replace with your directory PATH and use forward/slashes/

    mat = scipy.io.loadmat(dataDir + 'Data/data_PHY_23287_EW.mat')

    ag_data = mat['input_GM_23287_EW'][:, 0:3600]
    u_data = mat['target_X_23287_EW'][:, 0:3600, :]
    ut_data = mat['target_Xd_23287_EW'][:, 0:3600, :]
    utt_data = mat['target_Xdd_23287_EW'][:, 0:3600, :]
...
with tf.device('/device:GPU:1'):
    ...
    Loss = model.train(num_epochs=20000, batch_size=N_train, learning_rate=1e-3, bfgs=1)
    ...
    ag_pred = mat['input_pred_GM_23287_EW']
    u_pred = mat['target_pred_X_23287_EW']
    ut_pred = mat['target_pred_Xd_23287_EW']
    utt_pred = mat['target_pred_Xdd_23287_EW']
    ag_pred = ag_pred.reshape([ag_pred.shape[0], ag_pred.shape[1], 1])

    #################### Change the concatenation to meet dimensions of training set #########################
    # on original Zhang et al. data it is concatenating 6, 6, 3 = 15, e.g. ag_pred, ag_pred, ag_pred[0:3]
    '''X_pred = np.concatenate([ag_pred, ag_pred, ag_pred[0:3]], axis=0)[:, 0:3600, :] 
    y_pred_ref = np.concatenate([u_pred, u_pred, u_pred[0:3]], axis=0)[:, 0:3600, :]
    yt_pred_ref = np.concatenate([ut_pred, ut_pred, ut_pred[0:3]], axis=0)[:, 0:3600, :]
    ytt_pred_ref = np.concatenate([utt_pred, utt_pred, utt_pred[0:3]], axis=0)[:, 0:3600, :]'''

    X_pred = np.concatenate([ag_pred, ag_pred[0:4]], axis=0)[:, 0:3600, :] 
    y_pred_ref = np.concatenate([u_pred, u_pred[0:4]], axis=0)[:, 0:3600, :]
    yt_pred_ref = np.concatenate([ut_pred, ut_pred[0:4]], axis=0)[:, 0:3600, :]
    ytt_pred_ref = np.concatenate([utt_pred, utt_pred[0:4]], axis=0)[:, 0:3600, :]
    ###########################################################################################################
...
scipy.io.savemat(dataDir + 'Results/results_PhyCNN_23287_EW.mat',
